...
- **`src/middleware.ts`** only checks for the *presence* of the auth cookie to provide a fast redirect — it does not verify the JWT signature, since `jsonwebtoken`/`bcryptjs` rely on Node APIs unavailable in the Edge runtime. The actual verification (`getCurrentUserId` in `src/lib/auth.ts`) runs server-side in the dashboard layout and every API route, which is the real security boundary.
